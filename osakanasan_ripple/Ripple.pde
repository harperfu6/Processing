...
Ripple()
{
  image = loadImage("mizu.png");
  //image = loadImage("sakana.png");
  size(800, 533);
  //size(500, 350);
  ndata = new float[width][height];
  odata = new float[width][height];
  light = new PVector(0.1, 0.1, 0);
  light.normalize();
  rain = false;
}
 
void draw(PVector _location)
{
 //if(mousePressed)
  for(int x = -10; x < 10; x++)
    for(int y = -10; y < 10; y++)
      odata[(int)_location.x + x][(int)_location.y + y] = 0.1;//odata[mouseX + x][mouseY + y] = 0.1;
