...
void keyPressed() {
  switch(key) {
    ...
    case 'c': palette = (palette + 1) % 4; break; //様々な色
    case 'b': border = !border; dx=0; dy=0; break;
    case 'i': invert = !invert; break; //valの値の反転
    case 's': soft = (soft + 1) % 3; break; // 0: nosmooth, 1: smooth, 2: BLUR(平滑化)
    case '+': lim = min(lim+8, 255); break; //8ずつ増え，最大255
    case '-': lim = max(lim-8, 0); break; //8ずつ減り，最小0
    case CODED:
      switch(keyCode) {
